var fadeSlideShow_descpanel={controls:[['img/loader.gif',7,7],['img/loader.gif',10,11],['img/loader.gif',54,55]],fontStyle:'normal 11px Verdana',slidespeed:700}
...
if(setting.displaymode.type=="auto"){setting.displaymode.wraparound=true}
...
try{setting.onslide.call(slideshow,setting.$gallerylayers.eq(setting.fglayer).get(0),setting.curimage)}catch(e){}
...
setting.$slideButton=setting.$togglerdiv.find('.slideButton').data('action',1);setting.$next=setting.$togglerdiv.find('.next').data('action','next')
setting.$prev.add(setting.$next).add(setting.$slideButton).click(function(e){var $target=$(this);slideshow.navigate($target.attr('data-index'));e.preventDefault()})
